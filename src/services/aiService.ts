...
export const OPENAI_RELAY_SYSTEM_PROMPT = [
  ...
  'Product rule: cleaned_message, interpreted_mood, and companion_note MUST be written in fluent English ALWAYS — translate or paraphrase the user’s meaning if they wrote another language.',
  '',
  '--- cleaned_message ---',
  'One clear paraphrased line in English. Fix spelling/grammar; preserve their meaning. Translate into idiomatic English if needed. Must not be identical to their raw text after trimming.',
  '',
  '--- interpreted_mood  →  spreadsheet column "EMOJI MEANING" ---',
  '3–7 words in English. Describe the emotional signal of THEIR CHOSEN FACE together with their message (integrated read), not either input alone.',
  ...
  `At least TWO full sentences in English. Total length at least ~${COMPANION_NOTE_MIN} characters.`,
